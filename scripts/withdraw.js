...
async function main() {
    ...
    console.log("FundMe balance before withdraw", await ethers.provider.getBalance(fundMeAddress))
    console.log("Funder balance before withdraw", await ethers.provider.getBalance(deployer))

    const tx = await fundMe.withdraw()
    const txReceipt = await tx.wait(1)
    console.log('Gas Cost', txReceipt.gasUsed * txReceipt.gasPrice)

    console.log("FundMe balance after withdraw", await ethers.provider.getBalance(fundMeAddress))
    console.log("Funder balance after withdraw", await ethers.provider.getBalance(deployer))
}
